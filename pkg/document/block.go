...
func (r *Rect) Render() error {
	...
	// reduce rect by half a lineW to render border line inside rect
	if b == BorderOutside {
		r.doc.Rect(x1+halfLineW, y1+halfLineW, r.w-lineW, r.h-lineW, "D")
	}
	...
	if b != BorderOutside {
		...
		if b == BorderTop ||
			...
			b == BorderOpenBottom ||
			b == BorderOpenLeft {
			r.doc.Line(x1, y1+halfLineW, x2, y1+halfLineW)
		}
		...
		if b == BorderRight ||
			...
			b == BorderOpenBottom ||
			b == BorderOpenLeft {
			r.doc.Line(x2-halfLineW, y1, x2-halfLineW, y2)
		}
		...
		if b == BorderBottom ||
			...
			b == BorderOpenRight ||
			b == BorderOpenLeft {
			r.doc.Line(x1, y2-halfLineW, x2, y2-halfLineW)
		}
		...
		if b == BorderLeft ||
			...
			b == BorderOpenRight ||
			b == BorderOpenBottom {
			r.doc.Line(x1+halfLineW, y1, x1+halfLineW, y2)
		}
	}
